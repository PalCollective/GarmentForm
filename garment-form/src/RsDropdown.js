...
const RsDropdown = ({ id , value,label, items, onChange, placeholder }) => {
    ...
    return (
       ...
       <>
        <InputLabel  sx={{textAlign:'right', width:'100%', fontSize:'18.5px', fontWeight:'550'}}  >{label}</InputLabel>
        <Select
        fullWidth
        variant='standard'
        ...
        >
       {items.map(item => (
       <MenuItem sx={{ textAlign: 'right' }} value={item.value}>{item.label}</MenuItem>
    ))}
